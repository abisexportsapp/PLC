...
class Binstockreport extends StatefulWidget {
  final String selectedPlant;

  const Binstockreport({super.key, required this.selectedPlant});

  @override
  State<Binstockreport> createState() => _BinstockreportState();
}

class _BinstockreportState extends State<Binstockreport> {
  bool isLoading = true;
  List<Map<String, dynamic>> binStockData = [];
  ...
  Future<void> fetchBinStockData() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          "https://ibg-bi-subaccount-cf-dev-com-plt-bi-farms-manager.cfapps.ap11.hana.ondemand.com/Plc_Automation/SAPBinStock_DtlsSet?\$filter= Werks eq '${widget.selectedPlant}'",
        ),
      ...
      if (response.statusCode == 200) {
        ...
        if (data['d'] != null && data['d']['results'] is List<dynamic>) {
          binStockData = (data['d']['results'] as List<dynamic>)
                            ...
                            child: SingleChildScrollView(
                              child: Column(
                                children: [
                                  ...
                                  //         children: [
                                  //           _buildRow(
                                  //               'Plant Code:', data['werks']),
                                  //           _buildRow('Material Code:',
                                  //               data['matnr']),
                                  //           _buildRow('Material Description:',
                                  //               data['maktx']),
                                  //           _buildRow(
                                  //               'Silo Qty:', data['silo']),
                                  //           _buildRow(
                                  //               'Godown Qty:', data['godown']),
                                  //           _buildRow('Bin Qty:', data['bin']),
                                  //         ],
                                  //       ),
                                  //     ),
                                  //   );
            ...
            SingleChildScrollView(
              ...
              child: DataTable(
                ...
                columns: const [
                  ...
                  DataColumn(label: Text("Bin Qty.")),
                ],
                rows: binStockData.map((data) {
